...
void ProfilerGUI::TryLoadSymbols(AttachInfo* output)
{
	...
	while (retry--)
	{
		...
		try
		{
			// loadSymbols is single use: it asserts it has not opened the process yet.
			// Retrying on the same object would add to the half built module list the
			// failed attempt left behind, against a process handle dbghelp knows nothing
			// about. Start from a fresh one every time.
			output->sym_info = std::make_unique<SymbolInfo>();
	...
	while (true)
	{
		...
		case ThreadPicker::RUN:
			...
			catch (SleepyException &e)
			{
				captureWin->Hide();
				wxLogError("%ls\n", e.wwhat());
				wxMessageBox(L"Error: " + e.wwhat(), L"Profiler Error", wxOK | wxCENTRE,
							 threadpicker.get());
				continue;
			}
